...
### 結果（一般化II 局所OSS被検体, 2026-06-04, full-power）: **一般化成立 ＋ tradeoff 顕在化**

subject=gemma4:e4b（ollama, codex --oss）, judge=gpt-5.5, R=3, 576生成/576判定, **0エラー**。

`E_unsupported`（不可知）: **H1=0.000**, H_para=0.000, H0_ablate=0.042, H0_neutral=0.104, H_base_len=0.188, **H_contra=0.604（最大）**。

事前登録規則の適用:
- H1 − H_base_len CI **[−0.354, −0.062]**（0未満）→「generalizes」**成立**。
- H1 ≈ H_para（CI[0,0]）, H_contra 最大 → auto-verdict=`meaning_attributable`。
- R=3 full-power のため、これは検出力ではなく実効果。→ **効果は被検体 gemma に一般化した**。

**tradeoff_flag=True（事前予測どおり）**: answerable で H1 が E_correct=0.688（baseline 1.000）, E_over_refusal=0.312（baseline 0）。gemma は H1 を字義通り取り answerable 一般知識問題を過剰拒否。gpt-5.5 では出なかった副作用。

**結論（3被検体まとめ）**:
- 中核効果（指示が無根拠断定を↓・包絡線超え・言い換え頑健・H_contra で最悪化）は **gpt-5.5（確証）と gemma（full-power一般化）の両方で meaning_attributable**、spark でも方向一致（R=1で有意化せず）。→ **hyp-meaning は被検体を超えて一般化**。
- ただし**指示の有用性トレードオフは被検体依存**: gpt-5.5 は無害（correct 0.98/over-refusal 0）、gemma は高コスト（correct 0.69/over-refusal 0.31）。小型モデルほど「情報限定」指示を過剰適用する。
- 含意: 安全指示の正味の望ましさは被検体ごとに評価が必要。副作用（over-refusal/correctness）を同時測定しないと gemma のトレードオフは見えない（methodology の相補イベント原則の実証）。
